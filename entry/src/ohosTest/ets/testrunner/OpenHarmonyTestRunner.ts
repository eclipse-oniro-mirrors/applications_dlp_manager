...
async function addAbilityMonitorCallback(err): Promise<void> {
  hilog.info(0x0000, 'testTag', '%{public}s', 'addAbilityMonitorCallback : %{public}s', JSON.stringify(err) ?? '');
}
...
export default class OpenHarmonyTestRunner implements TestRunner {
  ...
  async onRun(): Promise<void> {
    hilog.info(0x0000, 'DlpManager', '%{public}s', 'DlpManager OnRun');
    ...
    hilog.info(0x0000, 'DlpManager', '%{public}s', 'DlpManager OnRun end');
  }
}